...
Comenzar();//de inicio semÃ¡ntico en caso de corresponder
Match(INICIO);
ListaSentencias();
Match(FIN);
}
...
switch ( tok ) {
 ...
 Match(ASIGNACION);
 Expresion(&der);
 Asignar(izq, der); //genera instrucciÃ³n de asignacion
 Match(PUNTOYCOMA);
 break;
...
switch ( estado )
{
 case 2 : if ( col != 11 ){ //si el carÃ¡cter espureo no es blancoâ€¦
